...
class AddMovieToCollectionUseCase(private val collectionsRepository: ICollectionsRepository) {

    suspend fun execute(collectionId: String, movieId: String): Flow<Result<Boolean>> {
        return collectionsRepository.addMovieToCollection(collectionId, movieId)
    }
}